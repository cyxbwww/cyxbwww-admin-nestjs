...
export class PermissionGuard implements CanActivate {
  constructor(private reflector: Reflector, private userServicese: UserService) {}
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    const permissionNames = await this.userServicese.findPermissionNames(token, request.user);

    const isContainedPermission = requiredPermissions.every(item => permissionNames.includes(item));
